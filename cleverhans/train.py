...
def train_simu(sess, loss1, loss2, x_train, y_train,
          init_all=False, evaluate=None, feed=None, args=None,
          rng=None, var_list1=None, var_list2=None, fprop_args=None, optimizer=None,
          devices=None, x_batch_preprocessor=None, use_ema=False,
          ema_decay=.998, run_canary=None,
          loss_threshold=1e5, dataset_train=None, dataset_size=None):
  """
  ...
  for device in devices:
    with tf.device(device):
      ...
      loss_value1 = loss1.fprop(x, y, **fprop_args)
      loss_value2 = loss2.fprop(x, y, **fprop_args)

      grads1.append(optimizer.compute_gradients(
          loss_value1, var_list=var_list1))
      grads2.append(optimizer.compute_gradients(
          loss_value2, var_list=var_list2))
  num_devices = len(devices)
  print("num_devices: ", num_devices)

  grad1 = avg_grads(grads1)
  grad2 = avg_grads(grads2)
  # Trigger update operations within the default graph (such as batch_norm).
  with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    train_step1 = optimizer.apply_gradients(grad1)
  with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    train_step2 = optimizer.apply_gradients(grad2)
  ...
  for epoch in xrange(args.nb_epochs):
    ...
    for batch in tqdm(range(nb_batches)):
      ...
      _, loss_numpy1 = sess.run(
          [train_step1, loss_value1], feed_dict=feed_dict)
      _, loss_numpy2 = sess.run(
          [train_step2, loss_value2], feed_dict=feed_dict)

      if np.abs(loss_numpy1) > loss_threshold:
        raise ValueError("Extreme loss during training: ", loss_numpy1)
      if np.isnan(loss_numpy1) or np.isinf(loss_numpy1):
        raise ValueError("NaN/Inf loss during training")

      if np.abs(loss_numpy2) > loss_threshold:
        raise ValueError("Extreme loss during training: ", loss_numpy2)
      if np.isnan(loss_numpy2) or np.isinf(loss_numpy2):
        raise ValueError("NaN/Inf loss during training")
...
def train_ftramer(sess, loss, x_train, y_train, attack_dict,
          init_all=False, evaluate=None, feed=None, args=None,
          rng=None, var_list=None, fprop_args=None, optimizer=None,
          devices=None, x_batch_preprocessor=None, use_ema=False,
          ema_decay=.998, run_canary=None,
  ...
  for device in devices:
    with tf.device(device):
      ...
      loss_value = 0.5 * (loss_value_1 + loss_value_2)
