...
def defence_frame(train_start=0, train_end=TRAIN_SIZE, test_start=0,
    ...
    if CHECK_DET == False:
        ...
        for i, attack_name in enumerate(FLAGS.attack_type):
            ...
            with sess.as_default():
                print(attack_name, 'model0', tf.reduce_mean(det_model.get_probs(origin_adv_x),0).eval(feed_dict={x:X_test}))
                print(attack_name, 'modeli', tf.reduce_mean(det_model.get_probs(modeli_adv_x),0).eval(feed_dict={x:X_test}))
                print(attack_name, 'ensemble', tf.reduce_mean(det_model.get_probs(ensemble_adv_x_P),0).eval(feed_dict={x:X_test}))
