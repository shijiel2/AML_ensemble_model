...
def train_ens(sess, loss, loss_2, x_train, y_train,
          init_all=False, evaluate=None, feed=None, args=None,
          rng=None, var_list=None, fprop_args=None, optimizer=None,
          devices=None, x_batch_preprocessor=None, use_ema=False,
          ema_decay=.998, run_canary=None,
  ...
  for device in devices:
    with tf.device(device):
      ...
      loss_value_2 = loss_2.fprop(x, y, **fprop_args)

      grads.append(optimizer.compute_gradients(
          0.5*(loss_value+loss_value_2), var_list=var_list))
  num_devices = len(devices)
  print("num_devices: ", num_devices)
  ...
  for device in devices:
    with tf.device(device):
      ...
      if x_train_adv_lst is not None:
        ...
        adv_x = tf.placeholder(x_train.dtype, (None,) + x_train.shape[1:])
      ...
      loss_value = loss.fprop(x, y, x_adv_lst=adv_x, **fprop_args)
